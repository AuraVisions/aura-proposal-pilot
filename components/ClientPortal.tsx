    ...
    <div className="max-w-6xl mx-auto space-y-6 md:space-y-10 pb-16">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <span className="bg-blue-600/30 text-blue-300 px-3 py-0.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] border border-blue-500/30">
          ...
          {proposal.status === ProposalStatus.COMPLETED && (
            ...
            >
              ...
              <span>{isExporting ? 'Preparing...' : 'Official PDF'}</span>
            </button>
          )}
        </div>
      </div>
        ...
        <div className="md:col-span-6 space-y-6">
          <div className="bg-white rounded-[1.5rem] md:rounded-[2.5rem] border border-slate-100 shadow-2xl shadow-slate-200/40 overflow-hidden min-h-[500px]">
            <div className="p-5 md:p-10 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-3 bg-slate-50/20">
              ...
              <div className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest self-start md:self-auto border ${
                activePhase.status === PhaseStatus.APPROVED ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-amber-50 text-amber-700 border-amber-100'
                ...
                <div key={fieldKey} className="space-y-3">
                  ...
                  <div className="text-slate-800 leading-relaxed text-sm md:text-base font-medium bg-slate-50/30 p-5 md:p-8 rounded-2xl border border-slate-100/50">
                    {activePhase.fields[fieldKey] ? (
                      ...
                      <em className="text-slate-300 font-normal">Details coming soon...</em>
                    )}
                  </div>
                </div>
              ))}
            ...
            {canAct && (
              <div className="p-6 md:p-10 bg-slate-900 border-t border-slate-800 flex flex-col gap-6 items-center">
                <div className="text-white/60 text-center space-y-1">
                  ...
                  <p className="text-xs md:text-sm">Approval will finalize and lock this phase content.</p>
                </div>
                <div className="flex gap-3 w-full max-w-sm justify-center">
                  <button onClick={() => setShowRejectModal(true)} className="flex-1 py-3 bg-transparent border border-slate-700 text-slate-400 rounded-xl font-black text-[10px] uppercase tracking-widest hover:text-white transition">Request Edits</button>
                  <button onClick={handleApprove} className="flex-1 py-3 bg-blue-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-500 shadow-xl transition active:scale-95">Approve Phase</button>
                </div>
              </div>
            )}
          </div>
                ...
                activePhase.logs.filter(l => ['APPROVE', 'REJECT', 'SUBMIT'].includes(l.action)).map((log) => (
                  <div key={log.id} className="border-l-2 border-slate-100 pl-4 py-0.5 space-y-1">
                    <div className="flex justify-between text-[9px] font-black uppercase tracking-widest text-slate-400">
                      <span className={log.action === 'APPROVE' ? 'text-emerald-500' : 'text-blue-500'}>{log.action}</span>
      ...
      {showRejectModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-slate-900/80 backdrop-blur-md">
          <div className="bg-white rounded-[2rem] max-w-lg w-full p-8 md:p-10 shadow-2xl border border-slate-200">
            <h3 className="text-2xl font-black text-slate-900 mb-1 tracking-tight">Request Refinement</h3>
            <p className="text-slate-500 mb-8 text-xs font-medium">Please detail the adjustments required for this phase.</p>
            <textarea
              autoFocus
              value={rejectFeedback}
              onChange={(e) => setRejectFeedback(e.target.value)}
              className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-5 text-sm md:text-base font-medium focus:bg-white focus:ring-4 focus:ring-rose-50 focus:border-rose-400 transition-all min-h-[160px] mb-6"
              placeholder="E.g. Please clarify the implementation timeline..."
            />
            <div className="flex space-x-3">
              <button onClick={() => setShowRejectModal(false)} className="flex-1 py-3 text-slate-400 font-black text-[10px] uppercase tracking-widest hover:text-slate-900 transition">Cancel</button>
              <button onClick={handleReject} className="flex-1 py-3 bg-rose-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-rose-700 transition active:scale-95">Send Feedback</button>
            </div>
          </div>
        </div>
      )}
